...
jobject NewJxpcapAddr(JNIEnv *env, jobject jxpcap_addresses, jobject jerrbuf, pcap_addr_t *addresses) {
	
	jobject obj = (*env)->NewObject(env, JxpcapAddrClass, JxpcapAddrInitMID);
	
	if(addresses->next != NULL) {
		jobject jxpcap_addr = NewJxpcapAddr(env, jxpcap_addresses, jerrbuf, addresses->next);
		if(jxpcap_addresses == NULL) {
			(*env)->DeleteLocalRef(env, jxpcap_addr);  
			return NULL;
		}
		(*env)->SetObjectField(env, obj, JxpcapAddrNextFID, jxpcap_addresses);
		if((*env)->CallBooleanMethod(env, jxpcap_addresses, ListAddMID, jxpcap_addr) == JNI_FALSE) {
